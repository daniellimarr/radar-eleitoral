...
export default function SuperAdminDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data, error } = await supabase
        .from("tenants")
        .select("id, name, status, contact_limit, created_at")
        .is("deleted_at", null)
  ...
  const statCards = [
    ...
    { label: "Gabinetes Suspensos", value: suspendedTenants, icon: AlertTriangle, bgColor: "bg-amber-100", iconColor: "text-amber-600" },
  ];
