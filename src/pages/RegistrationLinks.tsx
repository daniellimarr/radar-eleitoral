...
export default function RegistrationLinks() {
  ...
  const fetch = async () => {
    if (!tenantId) return;
    const { data } = await supabase.from("registration_links").select("*").eq("tenant_id", tenantId).order("created_at", { ascending: false });
    setLinks(data || []);
  };
  ...
  const handleCreate = async () => {
    ...
    const { error } = await supabase.from("registration_links").insert({
      tenant_id: tenantId, slug, coordinator_id: user?.id,
    });
    if (error) toast.error(error.message);
    else { toast.success("Link criado!"); setIsOpen(false); setSlug(""); fetch(); }
    setLoading(false);
  };
      ...
      <Card>
        <CardContent className="p-0">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>Link</TableHead><TableHead>Status</TableHead><TableHead>Criado em</TableHead><TableHead className="w-24">Ações</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {links.length === 0 ? (
                <TableRow><TableCell colSpan={4} className="text-center py-8 text-muted-foreground">Nenhum link</TableCell></TableRow>
              ) : links.map((l) => (
                <TableRow key={l.id}>
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      ...
                      <Button variant="ghost" size="icon" onClick={async () => { await supabase.from("registration_links").delete().eq("id", l.id); fetch(); }}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                    </div>
                  </TableCell>
                </TableRow>
              ))}
